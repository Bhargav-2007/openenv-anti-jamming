...
Defines typed Action, Observation, and State structures using openenv-core base classes.
"""
...
class AntiJammingAction(Action):
    ...
    - Anti-jamming spread-spectrum and filtering techniques
    """
...
class AntiJammingObservation(Observation):
    """
    What the agent observes from the environment.

    Inherits ``done`` and ``reward`` from ``openenv.core.Observation``.
    Includes:
    ...
    # Spectrum information (16 representative channels for LLM compatibility)
    channel_states: List[ChannelState] = Field(
        ...,
        max_length=16,
        description="Current state of up to 16 sampled channels",
    )
...
class AntiJammingState(State):
    ...
    Used by the ``state`` property and for debugging / grading.
    """
