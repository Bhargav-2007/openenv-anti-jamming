...
class SpotJammer(Jammer):
    ...
    def __init__(self, target_channel: int = 32, **kwargs) -> None:
    ...
    def step(self) -> None:
        ...
        self.attack_power = {self.target_channel: self.max_power_dbm}
...
class BarrageJammer(Jammer):
    ...
    def __init__(self, num_attack_channels: int = 16, **kwargs) -> None:
    ...
    def step(self) -> None:
        ...
        self.attack_power = {ch: power_per_channel for ch in self.current_channels}
...
class SweepJammer(Jammer):
    ...
    def __init__(self, sweep_width: int = 8, sweep_rate: int = 2, **kwargs) -> None:
    ...
    def step(self) -> None:
        ...
        self.current_channels = [
            (self.sweep_position + i) % self.num_channels for i in range(self.sweep_width)
        ]

        self.attack_power = {
            ch: self.max_power_dbm - 3 for ch in self.current_channels
        }
...
class ReactiveJammer(Jammer):
    ...
    def __init__(self, reaction_delay: int = 1, **kwargs) -> None:
    ...
    def step(self) -> None:
        ...
        if len(self.observed_tx_channels) >= self.reaction_delay:
            ...
            self.attack_power = {target_channel: self.max_power_dbm}
        else:
            target_channel = self.rng.randint(0, self.num_channels)
            self.current_channels = [target_channel]
            self.attack_power = {target_channel: self.max_power_dbm * 0.5}
...
class PulseJammer(Jammer):
    ...
    def __init__(self, duty_cycle: float = 0.5, pulse_channels: int = 8, **kwargs) -> None:
    ...
    def step(self) -> None:
        ...
        if is_on:
            ...
            self.attack_power = {ch: self.max_power_dbm for ch in self.current_channels}
        else:
            self.current_channels = []
            self.attack_power = {}
...
class SmartLearningJammer(Jammer):
    ...
    def __init__(self, learning_rate: float = 0.1, **kwargs) -> None:
    ...
    def step(self) -> None:
        ...
        if len(self.observed_tx_channels) > 0:
            for ch in self.observed_tx_channels[-5:]:
                self.channel_probability[ch] += self.learning_rate

            self.channel_probability /= self.channel_probability.sum()

        top_channels = np.argsort(self.channel_probability)[-4:]
        self.current_channels = top_channels.tolist()
        self.attack_power = {
            ch: self.max_power_dbm - 6 for ch in self.current_channels
        }
